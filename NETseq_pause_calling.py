...
def gini_boot_method(window, wsize, rng, n_boot = 10000):
    '''
    Calculate the sampling variation of the Gini coefficient through
    bootstrapping
    Arguments:
        window - (loc, array) tuple with location of window center and array of data
        wsize - int half the size of the window
        rng - np.random.generator
        n_boot - int number of bootstraps to do

    Returns:
        A list of information about the test including:

        coord, coord + 1, statistic, standard error, low confidence interval
        high confidence interval, # of zeros in window, total count in window
    '''
    loc, array = window
...
def stat_bootstrapped(array, true_val, stat_func, rng, n_boot=1000, alpha = 0.05):
    '''
    Calculate a null distribution for any stat through reshuffling of reads in 
    a window

    Assumes your null is a multinomial of all counts in the window across the
    size of a window

    Arguments:
        window - array array of data
        true_val - float actual value for the data
        stat_func - func for calculating the stat of interest
        rng - np.random.generator
        n_boot - int number of bootstraps to do
        alpha - for calculating quantile-based intervals

    Returns:
        A list of information about the test including:

        median of bootstrapped values, mad of bootstrapped values, low interval,
        high interval, and permutation test-based p-value
    '''
    total = int(np.sum(array))
    arr_size = len(array)
...
def stat_bootstrapped_compare(array, true_val, stat_func, compare_func, rng, n_boot=1000, alpha = 0.05):
    '''
    Calculate how different a stat is from a sampling-based multinomial null.

    Arguments:
        window - array array of data
        true_val - float actual value for the data
        stat_func - func for calculating the stat of interest
        compare_func - func for comparing actual to null
        rng - np.random.generator
        n_boot - int number of bootstraps to do
        alpha - for calculating quantile-based intervals
    ...
    Returns:
        ...
        median of bootstrapped values, mad of bootstrapped values, low interval,
        high interval, permutation test-based p-value, median of compared stat,
        mad of compared stat, low of compared stat and high of compared stat
    '''
    total = int(np.sum(array))
    arr_size = len(array)
    sample_data = rng.multinomial(total, [1/arr_size]*arr_size, size = n_boot)
...
def gini_null_method(window, wsize, rng, n_boot = 10000):
    '''
    Calculate how different a gini stat is from a sampling-based multinomial null.

    Arguments:
        window - array array of data
        wsize - half the size of a size of window
        rng - np.random.generator
        n_boot - int number of bootstraps to do
    ...
    Returns:
        ...
        coord, coord + 1, gini, median_null_gini, mad_null_gini, low null CI,
        high null CI, perm-based p-value, # of zeros in window, total count in window
    '''
...
def gini_null_ratio_method(window, wsize, rng, n_boot = 10000):
    ...
    Returns:
        ...
        coord, coord + 1, gini, median_null_gini, mad_null_gini, low null CI,
        high null CI, perm-based p-value, median_ratio_to_null, mad_ratio_to_null,
        low CI of ratio, high CI of ratio, 
        # of zeros in window, total count in window
    '''
    loc, array = window
    stat = gini_coefficient(array)
...
def augment_with_random_state(iterable, args):
    ...
    Returns
        a list of (iterable, random_seed, n_bootstraps)
    """
...
def augment_with_random_state_and_wsize(iterable, args):
    """ Zip an iterator with a np random seed and size of window
    ...
    Returns
        a list of (iterable, windowsize, rng, n_bootstraps)
    """
...
def multiprocess_bootstrap(window, rng, bootstraps):
    """ Helper function to call during multiprocessing for region-based
    estimates
    """
...
def run_tests_over_strand(arrays, methods, filters, initial_filters, args):
    """
    Run statistical tests over many arrays

    Arguments:
        arrays - dict of arrays with keys for each chrom
        methods - dict of function factory to apply to each array
        filters - list of filters of sites to skip testing on
        initial_filters - dict of starting filters for each chromosome
        args - arguments parsed from command line

    Output:
        list of arrays for each window with results from test
    """
    all_output = []
...
def genomewide_gini(args):
    """
    Main function for calculating gini coefficient genome-wide
    """
...
def genomewide_fast(args):
    """
    Main function for calculating pause calling genome-wide
    """
...
def region_main(args):
    """
    Main function for calculating pause calling in a region-specific manner.
    Uses a class structure that is slower than the genome-wide caller
    """
    import bed_utils
