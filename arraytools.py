...
def relative_summit_loc(array, wsize = 50):
    """
    Find the maximum of a gaussian-smoothed array

    Args:
        array - 1 dimensional numpy array
        wsize - wsize for smoothing
    """
...
def traveling_ratio(array, wsize = 50, peak = None, out = "ratio"):
    """
    Find the traveling ratio of a signal

    Args:
        array - 1 dimensional numpy array
        wsize - half the size of the window for window calcs
        peak - location of peak for window A. Dynamically found if None
        out - one of "ratio", "A", "B" for what to output
    """
...
def to_lower_resolution(array, res_scale_factor = 5, summary_func = np.nanmean): 
    """
    Convert an array to a lower resolution

    Args:
        array - 1 dimensional numpy array
        res_scale_factor - amount to scale the array down to
        summary_func - function to apply to windows that are collapsed
    """
    size = len(array)
